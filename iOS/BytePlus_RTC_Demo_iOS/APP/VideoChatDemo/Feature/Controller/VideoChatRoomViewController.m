...
- (void)receivedFinishLive:(NSInteger)type roomID:(NSString *)roomID {
    ...
    [self hangUp:NO];
    if (type == 3) {
        [[ToastComponents shareToastComponents] showWithMessage:LocalizedString(@"Room has been closed due to violations of Terms of Service.") delay:0.8];
    }
...
- (void)receivedJoinInteractWithUser:(VideoChatUserModel *)userModel
    ...
    if ([userModel.uid isEqualToString:[LocalUserComponents userModel].uid]) {
        ...
        [[VideoChatRTCManager shareRtc] enableLocalAudio:self.settingCompoments.mic];
        [[VideoChatRTCManager shareRtc] enableLocalVideo:self.settingCompoments.camera];
        [[ToastComponents shareToastComponents] showWithMessage:LocalizedString(@"You have been a guest\0")];
    }
...
- (void)receivedAnchorPKReply:(VideoChatPKReply)reply
    ...
    if (reply == VideoChatPKReplyAccept) {
        ...
        [[ToastComponents shareToastComponents] showWithMessage:[NSString stringWithFormat:LocalizedString(@"%@ declined your invitation"), anchorModel.name]];
    }
}
...
- (void)receivedAnchorPKEnd {
    if ([self isHost]) {
        if (self.pkComponents.activeEndPK) {
            [[ToastComponents shareToastComponents] showWithMessage:[NSString stringWithFormat:LocalizedString(@"You have disconnected with %@"), self.pkComponents.anchorModel.name]];
        } else {
            [[ToastComponents shareToastComponents] showWithMessage:[NSString stringWithFormat:LocalizedString(@"%@ has left"), self.pkComponents.anchorModel.name]];
        }
    ...
    if (status == VideoChatRoomBottomStatusInput) {
        ...
        if ([self isHost]) {
            if (self.pkComponents.anchorModel) {
                [[ToastComponents shareToastComponents] showWithMessage:LocalizedString(@"A host is in your room. Failed to add a guest")];
            }
            else {
...
- (void)setChatRoomMode:(VideoChatRoomMode)chatRoomMode {
    ...
    self.seatCompoments.hostUserModel = self.hostUserModel;
    ...
    if (chatRoomMode == VideoChatRoomModeChatRoom && [self isHost]) {
        ...
        if (![self isHost]) {
            ...
            self.seatCompoments.loginUserModel.status = VideoChatUserStatusDefault;
        }
    }
}
