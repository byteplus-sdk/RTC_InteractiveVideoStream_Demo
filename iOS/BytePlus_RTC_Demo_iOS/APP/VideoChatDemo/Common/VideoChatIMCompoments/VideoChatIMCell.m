...
- (void)setModel:(VideoChatIMModel *)model {
    ...
    if (NOEmptyStr(model.message)) {
        ...
        NSString *unitStr = model.isJoin ? LocalizedString(@"%@ entered the room") : LocalizedString(@"%@ left the room");
        NSString *roomName = [NSString stringWithFormat:unitStr, model.userModel.name];
        [self setLineSpace:5
                  withText:roomName
                   inLabel:self.roomNameLabel];
    }
