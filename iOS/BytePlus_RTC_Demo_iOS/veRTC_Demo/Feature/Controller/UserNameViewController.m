...
- (BaseButton *)rightButton {
    if (!_rightButton) {
        ...
        [_rightButton setTitle:LocalizedString(@"OK") forState:UIControlStateNormal];
    }
    return _rightButton;
}
...
- (UILabel *)navLabel {
    if (!_navLabel) {
        ...
        _navLabel.text = LocalizedString(@"Change your username");
    }
    return _navLabel;
}
...
- (MenuCreateTextFieldView *)userNameTextFieldView {
    if (!_userNameTextFieldView) {
        ...
        _userNameTextFieldView.placeholderStr = LocalizedString(@"Username\0");
