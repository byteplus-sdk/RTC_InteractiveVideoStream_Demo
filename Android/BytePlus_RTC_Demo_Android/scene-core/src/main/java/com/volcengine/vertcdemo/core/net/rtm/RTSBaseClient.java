...
public abstract class RTSBaseClient {
    ...
    public RTSBaseClient(@NonNull RTCVideo video, @NonNull RTSInfo rtmInfo) {
        mRTCVideo = video;
        mRtmInfo = rtmInfo;
        initEventListener();
    }
     ...
     * @param callback    回调接口
     */
    public <T extends RTSBizResponse> void sendServerMessage(String eventName, String roomId, JsonObject content, Class<T> resultClass, IRequestCallback<T> callback) {
     ...
     */
    public <T extends RTSBizResponse, D extends RTSBizInform> void onMessageReceived(String uid, String message) {
        try {
            ...
            if (TextUtils.equals(messageType, ServerResponse.MESSAGE_TYPE_RETURN)) {
                ...
                RTSRequest request = TextUtils.isEmpty(requestId) ? null : mCallBacksWithRequestId.get(requestId);
    ...
    private <T extends RTSBizResponse> void notifyRequestSuccess(IRequestCallback<T> callback, T data) {
        AppExecutors.mainThread().execute(() -> {
            if (callback == null) {
                return;
            }
